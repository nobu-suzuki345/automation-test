...
- 🌦️ **天気** … Open-Meteo（APIキー不要）。現在地 or 都市名検索。**週間予報**付き
- 📅 **今日の予定** … Google カレンダーから取得。Meet / Zoom の参加ボタン付き
- 🗓️ **今後の予定** … 今日以降7日間の予定をまとめて表示
- ✅ **TODO** … localStorage 保存。期限・**カテゴリ**・**ドラッグ並び替え**＋「カレンダーに追加」
- 🍅 **ポモドーロ** … 作業25分 / 休憩5分の集中タイマー。終了時にブラウザ通知＋ビープ
- 📝 **メモ** … **複数ノート**対応のクイックノート（自動保存）
- ⚙ **カスタマイズ** … カードの表示/非表示・並び替え（**ドラッグ＆ドロップ**対応）

### スケジュール帳
- 月 / 週ビューの切り替え（前後ナビ付き）
- **複数カレンダーの統合表示・切替**（📚 から表示するカレンダーを選択、色で識別）
- 予定の追加先カレンダーを選択可能
- 日付クリックでその日の予定を表示
- 予定の **追加 / 編集 / 削除**（Google カレンダーに反映）
- 予定の **色分け**（Google カレンダーの 11 色に対応・表示/設定）
